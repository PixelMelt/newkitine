...
function phrases(text) {
  return text.toLowerCase().split('|').map((phrase) => phrase.trim()).filter(Boolean);
}

export const emptyFilters = () => ({
  include: '', exclude: '', type: '', size: '', bitrate: '', duration: '', freeSlot: false,
});
...
export function compileFilters(f) {
  ...
  clauses.push(...numericClauses(
    f.bitrate, ({ file }) => file.attributes?.bitrate ?? 0, Number, false));
  clauses.push(...numericClauses(
    f.duration, ({ file }) => file.attributes?.length ?? 0, parseDuration, false));
  if (f.freeSlot) {
