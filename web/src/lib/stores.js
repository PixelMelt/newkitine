...
export const interests = writable({
  liked: [], hated: [], recommendations: [], unrecommendations: [],
  recommendations_for: null, recommendations_global: false,
  similar_users: [], similar_users_for: null,
});
  ...
  switch (msg.type) {
    ...
    case 'transfers_cleared': {
      ...
      const cleared = (status) =>
        msg.statuses.some((s) => (s === 'failed' ? status.startsWith('failed') : status === s));
      store.update((map) => {
        const kept = {};
    ...
    case 'room_user_joined':
      rooms.update((r) => {
        ...
        if (room && !room.users.includes(msg.username)) {
          ...
          room.messages = [...room.messages.slice(-199), {
            status: true,
    ...
    case 'room_user_left':
      rooms.update((r) => {
        ...
        if (room) {
          ...
          room.messages = [...room.messages.slice(-199), {
            status: true,
            message: `${msg.username} left the room`,
            timestamp: Date.now() / 1000,
          }];
        }
        return r;
...
export async function loadInitialState() {
  const [
    statusData, searchList, downloadData, uploadData, buddyData,
    bannedData, ignoredData, wishlistData, interestsData, roomsData, chatsData,
    settingsData,
  ] = await Promise.all([
    apiGet('/status'), apiGet('/searches'), apiGet('/downloads'), apiGet('/uploads'),
    apiGet('/buddies'), apiGet('/banned'), apiGet('/ignored'), apiGet('/wishlist'),
    apiGet('/interests'), apiGet('/rooms'), apiGet('/chats'), apiGet('/settings'),
  ]);
  ...
  const full = await Promise.all(
    searchList.searches.map((s) => apiGet(`/searches/${s.token}`)));
  searches.set(full);
